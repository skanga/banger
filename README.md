...
Python flow queries bind explicit arguments using indexed signatures, including positional-only, keyword-only and variadic parameters. Ordinary implicit-receiver calls retain their candidate resolution evidence. Dynamic splats and signatures hidden by decorators remain unresolved; omitted default expressions are not traced. Other languages currently use positional argument approximations.
