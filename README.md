...
`search_text` finds literal text in discoverable workspace files without shell execution, including in read-only mode. It supports file/directory scope and optional case-insensitive Unicode matching, returns physical line numbers, and honors Git ignores and excluded directories. It reports skipped binary/non-UTF-8/oversized files and bounded previews/results. Previews include the first match where space permits, with 1-based source character columns, an exclusive end column and the preview starting column. Searches are limited to 2 MiB per file, 32 MiB read per query and 10,000 discovered files; set `regex=True` for Python regular expressions with a 10-second isolated-worker deadline. Regex timeout returns an error; narrow the path or simplify the pattern before retrying.
...
`list_dependencies` reads declared dependency groups from discoverable `pyproject.toml`, requirements text files, `package.json`, `Cargo.toml`, `go.mod`, Maven POMs, and .NET project/central-package files. It preserves declaration text and reports recognized unsupported or malformed manifests. It does not install packages, evaluate build scripts, expand conditions/properties/includes, or resolve installed/transitive versions. Reads are bounded to 100 recognized manifests, 1 MiB per file and 4 MiB per query.
